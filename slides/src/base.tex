...
\begin{frame}{Мотивация}
  ...
  \begin{center}
    ...
    \medskip
    \visible<2->{Только если $x$ и $y$ не могут быть синонимами.}
  \end{center}
...
\begin{frame}{Недостатки предыдущей работы}
  ...
  \begin{center}
    {\large $a$ и $b$~--- могут ли быть синонимами?}
  \end{center}

  \begin{columns}[t]
    \begin{column}{0.5\textwidth}
      \begin{block}{Проблема с переменными}
        \begin{algorithmic}[1]
          \State $x\visible<2->{_1} \gets \op{new}~\type{T}$
          \State $a \gets x\visible<2->{_1}$
          \State $x\visible<2->{_2} \gets \op{new}~\type{T}$
          \State $b \gets x\visible<2->{_2}$
        \end{algorithmic}
      \end{block}

      \medskip
      \visible<3->{\centerline{Нет.}}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{block}{Проблема с полями}<4->
        \begin{algorithmic}[1]
...
\begin{frame}{Представление целей указателей}

  \begin{block}{Внутреннее представление}
    \begin{algorithmic}
      \State $\M_0 \gets \op{initialmemory}$
        \visible<3->{\Comment $\M_0 = \{\Mfield{fields}\colon \emptyset\}$}
      \State $a \gets \op{new}~\type{A}$
      ...
      \State
        ...
        \State $\M_2 \gets \op{putfield}(\M_0, a, \field{f}, x_2)$
          \visible<3->{\Comment $\M_2 = \{\Mfield{fields}\colon
            \{ O_1.\field{f} \Rightarrow \{ O_{global} \} \}\}$}
      \State
      \State $\M_3 \gets \op{phi}(\M_1, \M_2)$
        \visible<3->{\Comment $\M_3 = \{\Mfield{fields}\colon
          \{ O_1.\field{f} \Rightarrow \{ O_2, O_{global} \} \}\}$}
...
\begin{frame}{Представление целей указателей}
  ...
  \begin{align*}
    \M = \{&\\
    &\Mfield{fields} \text{~--- множество целей полей абстрактных объектов},\\
    &\Mfield{statics} \text{~--- множество целей статических полей},\\
    &\Mfield{shared} \text{~--- множество разделяемых объектов},\\
    &\Mfield{escaped} \text{~--- множество утекающих объектов}.\\
    \}&
  \end{align*}

  \[\pts{x} \text{~--- множество целей переменной.}\]
...
\begin{frame}{Алгоритм анализа}
  ...
  выражения ссылочного типа $a$ и $b$ могут являться синонимами тогда и только
  тогда, когда их множества целей имеют непустое пересечение
    ...
    \begin{column}{0.5\textwidth}
      \visible<2->{\begin{block}{Избыточные чтения}
        \begin{algorithmic}[1]
          \State $x.\field{f} \gets 42$
          \State $y.\field{f} \gets 37$
          \State $u \gets \sout<3->{x.\field{f}}
                          \only<3->{\ 42}$
        \end{algorithmic}
      \end{block}}
    \end{column}
    \begin{column}{0.5\textwidth}
      \visible<4->{\begin{block}{Взрыв объектов}
        \begin{algorithmic}[1]
          \State $\sout<5->{r \gets \op{new}~\type{Rect}(a, b)}$
          \State $u \gets \sout<5->{r.\op{area}()}
                        \ \only<5->{a \cdot b}$
        \end{algorithmic}
      \end{block}}
    \end{column}
